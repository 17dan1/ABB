/*
   ...
   ABB Power meter is configured to output 1 10ms pulse every Wh consumption, 2 channer is used,
   one for active and the other for reactive energy. This script uses TIMER 1 and EXTERNAL INTERRUPT
   to count the time between every pulse end, measure the time interval to extract and print to LCD the power
   consumption in kW and kVA.

   UPLOAD: To upload pure C Arduino code to the Arduino Micro (ATmega32U8), press the upload button, 
   then, during the upload phases, reset the microcontroller with the reset button.

   -- PIN MAPPING --
   PIN    PORT      USAGE
   2      PD1       INT1 input reactive energy pulse
   3      PD0       INT0 input active energy pulse
   6      PD7       INT6 bachlight pushbutton
   7      PE6       Backlight LED
   13     PC7       LED_BUILDIN
...
volatile uint8_t backlight_time_count = 0;
...
volatile bool backlight_flag = true;
...
int main() {
  ...
  PORTD = (1 << PD7);  // backlight_flag ON
  DDRC = 0b10000000;  // Set LED_BUILDIN as output
  // -- EXTERNAL INTERRUPT SETUP --
  cli();
  EICRA = 0; EIMSK = 0;  // Clear external interrupts 1 control registers
  EICRA |= 0b00001010;  // Falling edge on INT1 and INT0
  EICRB &= ~(0b00110000);  // Low value on INT6
  EIMSK |= (1 << INT0) | (1 << INT1) | (1 << INT6); // Enable external interrupt INT1 and INT0
  EIFR &= ~((1 << INTF0) | (1 << INTF1) | (1 << INTF6)); //External Interrupt Flag Register cleared
  // -- TIMER 1 SETUP --
  ...
  send_data(0, 0b00000010);   // 4 bit mode 0b00100000 swapped nibbles to work with 2° set in 8 bit mode
